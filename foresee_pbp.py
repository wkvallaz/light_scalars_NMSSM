...
##########################################################
####/                                                \####
###/     foresee_pbp IS THE MODIFIED VERSION OF MY    \###
##|     ALREADY-MODIFIED VERSION OF FORESEE, WHICH     |##
##|     IS WRITTEN TO TAKE INPUTS POINT-BY-POINT       |##
##|     INSTEAD OF INPUTS AS VALUES ON A LATTICE       |##
##|     - Wolfgang Vallazza-Margl, 29MAY24             |##
###\                                                  /###
####\================================================/####
####/================================================\####
###/                                                  \###
##|     foresee.py wei_set_ctau_2d takes inputs        |##
##|     as a list of masses, a list of couplings,      |##
##|     which serve as the grid markers, and list      |##
##|     of ctaus (lifetimes at each gridpoint),        |##
##|     it wants to use an interpolate function        |##
##|     which is why inputs have to be on a grid.      |##
##|     to go off-grid, the interpolate function       |##
##|     would be used, but they've proven              |##
##|     unreliable (both in values, and bugcauser)     |##
##|     so in order to never interpolate, change       |##
##|     the way this is gotten. if providing long      |##
##|     list of true events, don't need interp- fn     |##
##|     just need a function to return lifetime        |##
##|     for that associated point (mass, params).      |##
##|                                                    |##
##|     I anticipate that this will require me to      |##
##|     change the way <couplings> moves around,       |##
##|     the way that <masses> moves around, or         |##
###\    within methods how they're looped over...     /###
...
### 
    def same(self,a,b):
        EPS = 0.0001 # fractional tolerance
        return (1-EPS)*a < b and b < (1+EPS)*a
###
###
######
    ###############################
...
class Model(Utility):
    ...
    def ctau_function_pbp(self,mass,tanb):
        # if tanb==3.4000000000000004: tanb=3.4
        # thisthing = tanb
        # tanb = np.float64(thisthing)
        # mass_slice = 
        
        # print(f"\nmass {round(mass,2)}\ttanb {tanb}\n")
        # print(f"-----\nmass slice\n {mass_slice}")

        # mass_slice = self.events_list.loc[self.events_list['mass']>.9999*mass] 
        # masz_slice = mass_slice.loc[mass_slice['mass']<1.0001*tanb]
        # tanb_slice = masz_slice.loc[masz_slice['tanb']>.9999*tanb] # doing this way bc
        # tanc_slice = tanb_slice.loc[tanb_slice['tanb']<1.0001*tanb]# float precision

        mass_slice = self.events_list.loc[self.events_list['mass']==mass] 
        tanb_slice = mass_slice.loc[mass_slice['tanb']==tanb] 
        # print(f"\nmass {mass}\ttanb {tanb}\t{tanb_slice['ctau']}\n")
        return tanb_slice['ctau'].values[0]
        
        # return ctau                                                       #\##\##\
        ## return 1e10 # if you ask for a point (mass, tanb) which is not   ##\##\##

    def set_ctau_pbp(self,filename):    #                                   \#\##\##
    ...
    def wei_set_ctau_2d(self,filename0,filename1,filename2,kind="linear"):
        ctau=np.loadtxt(filename0)
        mass=np.loadtxt(filename1)
        tanb=np.loadtxt(filename2)
        self.ctau_coupling_ref=None
        self.ctau_function=interpolate.interp2d(mass,tanb,ctau,kind=kind)
        
    def set_ctau_2d(self,filename):
        data=self.readfile(self.modelpath+filename).T
        self.ctau_coupling_ref=None
        ### Then use bisplev to do the interpolation: bisplev(xs, ys, tck, ...) where xs, ys are the coords in interpolate and tck is the output from bisplrep.
        ## wolf
        #tck = interpolate.bisplrep(data[0], data[1], data[2])
        #self.ctau_function=interpolate.bisplev(data[0],data[1],tck)
        #pass
        ## below was the old remnants
        try: #                                                                               fill_value="extrapolate" / None,  no s arg
            self.ctau_function=interpolate.interp2d(data[0], data[1], data[2], kind="linear")#,fill_value="extrapolate")#         ,s=len(data[0]))
        except:
    ...
    def write_gammas(self,tag):
        dirname = self.modelpath
        filename = dirname+"gammas/"+self.model_name+tag+"_gammas.txt"
        
        f = open(filename, "w")
        for i,g in enumerate(self.gamma_list):
            line = f"{g}\n"      
            f.write(line)              
        f.close()
    ...
    def get_ctau(self,mass,coupling):
        if self.ctau_function==None:
            ...
            ret = self.ctau_function(mass,coupling)
            return ret
            # if ret[0]<1e-500:
            #     ret[0]=1e-500
            # return ret
            # return [max(self.ctau_function(mass,coupling)[0],1e-20)] # by default, doesn't have this max( xxxxx , 1e-20) that i want to introduce, should just be selfcall 
        else:
#            return self.ctau_function(mass) / coupling**2 *self.ctau_coupling_ref**2 #i think for 1d inputs, this needs be reversed, since @ hi coup, ctau inc (hi tanb == hi life)
            return self.ctau_function(mass) * coupling**2 /self.ctau_coupling_ref**2 # my inversion
    ...
    def set_br_1d(self,modes, filenames, finalstates=None):
        ...
        for channel, filename, finalstate in zip(modes, filenames, finalstates):
           ...
           ### print(channel,filename,finalstate)
            self.br_functions[channel] = function
            self.br_finalstate[channel] = finalstate
    
    def wei_set_br_2d(self,modes,filenames,filename1,filename2):
        self.br_mode="2D"
        self.br_functions = {}
        # ctau=np.loadtxt(filename)
        mass=np.loadtxt(filename1)
        tanb=np.loadtxt(filename2)
        for channel, filename in zip(modes, filenames):
            data = np.loadtxt(filename)
            function = interpolate.interp2d(mass, tanb, data, kind="cubic")
            self.br_functions[channel] = function
    ...
    def get_production_scaling(self, key, mass, coupling, coupling_ref, Acomp, Acomp_ref, mHpm, mHpm_ref):
        scaling = self.production[key]["scaling"]
        #coupling = coupling**-1
        if False: # For viewing values in the return - debugging
            print("mass:",mass)
            print("key:",key)
            print("coup:",coupling)
            print("coup_ref:",coupling_ref)
            print("selfprodkeyBR:",self.production[key]["br"])
            print("   ^^\{coup\}:",self.production[key]["br"], {"coupling":coupling}     ) 
            #print( eval(self.production[key]["br"], {"coupling":coupling})/eval(self.production[key]["br"], {"coupling":coupling_ref}) )
        
        if self.production[key]["type"] in ["2body","3body"]:
            if scaling == "manual":
                
                #num = eval(self.production[key]["br"], {"coupling":coupling})
                num = eval(self.production[key]["br"])
                #print("num",num)
                
                #den = eval(self.production[key]["br"], {"coupling":coupling_ref})
                den = eval(self.production[key]["br"].replace("coupling","coupling_ref").replace("Acomp","Acomp_ref").replace("mHpm","mHpm_ref"))
                #print("den",den)
        ...
        if self.production[key]["type"] == "mixing":
            ...
            if scaling == "manual":
                ...
                #num = eval(self.production[key]["mixing"], {"coupling":coupling})**2
                num = eval(self.production[key]["mixing"])**2
                #print("num",num)
                
                #den = eval(self.production[key]["mixing"], {"coupling":coupling_ref})**2
                den = eval(self.production[key]["mixing"].replace("coupling","coupling_ref").replace("Acomp","Acomp_ref").replace("mHpm","mHpm_ref"))**2
                #print("den",den)
...
class Foresee(Utility):
    ...
    def make_spectrumplot(self, list_t, list_p, list_w, prange=[[-5, 0, 100],[ 0, 4, 80]], vmin=None, vmax=None):
        ...
        h=ax.hist2d(x=list_t,y=list_p,weights=list_w,
                    ...
                    norm=matplotlib.colors.LogNorm(vmin=vmin, vmax=vmax), cmap="hsv",
        )
    ...
    def convert_to_hist_list(self,momenta,weights, do_plot=False, filename=None, do_return=False, prange=[[-5, 0, 100],[ 0, 4, 80]], vmin=None, vmax=None, suppress_outs = False):
        ...
        if filename is not None:
            if not suppress_outs: print ("save data to file:", filename)
    ...
    def decay_in_restframe_2body(self, br, m0, m1, m2, nsample):
        ...
        for i in range(nsample):
            ...
            if False and i==0: print("\ndecay_in_restframe_2body\tweights.append\t{}".format(br/nsample)) # DEBUGGING, why do BRs blow up?
    ...
    def decay_in_restframe_3body(self, br, coupling, m0, m1, m2, m3, nsample):
        ...
        for i in range(nsample):
            ...
            if False and i==0: print("\ndecay_in_restframe_3body\tweights.append\t{}".format(brval)) # DEBUGGING, why do BRs blow up?
            #save
    ...
    def boostlist(arr_particle, arr_boost):
        ...
        gamma_list = np.zeros(len(arr_particle)*len(arr_boost)) # my addN to scale ctau to lab frame
        ...
        for bx, by, bz in arr_boost:
            ...
            for xx, xy, xz, xt in arr_particle:
                ...
                out[i,0]= th
                out[i,1]= pm
                gamma_list[i] = gamma # my addN
                # if i%10000==0: print("gamma\t",gamma,"\tgamma2\t",gamma2)
                i+=1
        return out, gamma_list
        
    def coord(self,mom):
        return np.array( [np.arctan(mom.pt/mom.pz), mom.p] )

    def get_llp_spectrum(self, mass, coupling, Acomp, mHpm, channels=None, do_plot=False, save_file=True,suppress_outs=False):
        ...
        for key in model.production.keys():
            ...
            if model.production[key]["type"] in ["2body", "3body"]:
                ...
                if model.production[key]["type"] == "3body":
                    ...
                    momenta_llp, weights_llp = self.decay_in_restframe_3body(eval(br), coupling, m0, m1, m2, m3, nsample)
                                                ##              used to just pass br here, not eval(br) and in d_i_rf_3b there was simply eval(br) there
                ...
                momenta_lab,gamma_list = self.boostlist(arr_momenta_llp, arr_minus_boostvectors) # gamma_list my addN
                # model.gamma_list.append(sum(gamma_list)/len(gamma_list))
                model.gamma_list=gamma_list
                model.write_gammas(f"_{key}_m_{mass}")
                # weights
            ...
            if save_file==True:
                for iproduction, production in enumerate(model.production[key]["production"]):
                    filenamesave = dirname+energy+"TeV_"+key+"_"+production.replace("<","LT")+"_m_"+str(mass)+".npy"
                    self.convert_to_hist_list(momenta_lab, weights_lab[:,iproduction], do_plot=False, filename=filenamesave,suppress_outs=suppress_outs)
        ...
        #return
        if do_plot:
            return self.convert_to_hist_list(momenta_lab_all, weights_lab_all, do_plot=do_plot)[0],model.gamma_list
    ...
    def get_events(self, mass, energy,
        ...
        for key in modes.keys():
            ...
            dirname = self.model.modelpath+"model/LLP_spectra/"
            filename = [dirname+energy+"TeV_"+key+"_"+production.replace("<","LT")+"_m_"+str(mass)+".npy" for production in modes[key]]
                                
            # try Load Flux file
            DEBUG = False
            try:
                if DEBUG: print("load", filename) #the m=1 LLP spectra files
                ...
                if DEBUG: print("Warning: file ",filename," not found")
                continue
            # gammas = []
            # try: # try load my gammas file
            #     if DEBUG: print("load", self.model.modelpath+model.model_name+f"_{key}_m_{mass}_gammas.txt") #the m=1 LLP spectra files
            #     with open(self.model.modelpath+model.model_name+f"_{key}_m_{mass}_gammas.txt") as f:
            #         f_reader = csv.reader(f,delimiter=" ")
            #         for i,r in enumerate(f_reader):
            #             gammas.append(float(r))
            #     f.close()
            # except:
            #     if DEBUG: print("Warning: file ",self.model.modelpath+model.model_name+f"_{key}_m_{mass}_gammas.txt"," not found")
            #     continue
            ...
            for p,w in zip(particles_llp,weights_llp):
                ...
                #loop over couplings
                for icoup,coup in enumerate(couplings): #PBP by the time we get to here, this should only be ONE coupling at this mass value
                    #add event weight                   # when calling get_events, at the chosen mass just send [tanB] with the one relevant value?
                    ...
                    try:
                        ...
                        prob_decay = 0
                    couplingfac = model.get_production_scaling(key, mass, coup, coup_ref, Acomp, Acomp_ref,mHpm,mHpm_ref)

                    try:
                        nsignals[icoup] += weight_event * couplingfac * prob_decay * br
                    except:
                        nsignals[icoup] += weight_event *0.
                    ...
                    try:
                        ...
                        stat_w[icoup].append([0. for _ in weight_event])
               ...
               #           p.p," <p.p\n\t",
                          ...
                          couplingfac," <couplingfac\n\t",
                          prob_decay," <prob_decay\n\t",
                          br," <br\n\t",
               #           dbar," <dbar\n\t",
                          stat_w[icoup][-1], " <stat_w[icoup][-1]\t",
               #           self.distance," <distance\n\t",
               #           self.distance_prod," <offset\n\t",
               #           self.length," <length\n\t",
               #           self.distance/dbar," <distance/dbar\n\t",
               #           self.distance_prod/dbar," <offset/dbar\n\t",
               #           self.length/dbar," <length/dbar\n\t",
               #           math.exp(-(self.distance))," <e-dist\n\t",
               #           math.exp(-(self.distance+self.length)),"<e-(dist+len)\n\t",
               #           math.exp(-(self.distance)/dbar)," <e-(dist/dbar)\n\t",
               #           math.exp(-(self.distance+self.length)/dbar),"<e-((dist+len)/dbar)\n\t",
                        )
        ...
        for key in modes.keys():
            ...
            for p,w in zip(particles_llp,weights_llp):
                ...
                for icoup,coup in enumerate(couplings):
                    ...
                    couplingfac = model.get_production_scaling(key, mass, coup, coup_ref, Acomp, Acomp_ref)
    ...
    def write_events(self, mass, coupling, Acomp, mHpm, energy, filename=None, numberevent=10, zfront=0, nsample=1, seed=None,
        ...
        _, _, _, weighted_raw_data, weights = self.get_events(mass=mass, energy=energy, couplings = [coupling], Acomp=Acomp, mHpm=mHpm, nsample=nsample, modes=modes, extend_to_low_pt_scales=extend_to_low_pt_scales, preselectioncuts=preselectioncuts)
    ...
    def plot_reach(self,
            ...
            fs_label=14,xscale="log",yscale="log"
        ):

        if "tan" in ylabel and "beta" in ylabel: yexp = -1  # y exponent (for inverting
        else: yexp = 1                                      #  coupling = cotB -> tanB

        # initiate figure
        matplotlib.rcParams.update({'font.size': 15})

        if branchings is None:
            fig, ax = plt.subplots(figsize=figsize,dpi=100) #this was at 400
        else:
        ...
        for bound in bounds2:
            ...
            y_coup = data.T[1]
            y_arr = [val**yexp for val in y_coup]
            
            ax.fill(data.T[0], y_arr, color="#efefef",zorder=zorder)
            ax.plot(data.T[0], y_arr, color="darkgray"  ,zorder=zorder,lw=1)
            zorder+=1
        ...
        for projection in projections:
            ...
            y_coup = data.T[1]
            y_arr = [val**yexp for val in y_coup]
            ax.plot(data.T[0], y_arr, color=color, ls="dashed", zorder=zorder, lw=1)
            zorder+=1
        ...
        for bound in bounds:
            ...
            data=self.readfile(self.model.modelpath+"model/lines/"+filename)
            y_coup = data.T[1]
            if "charm-tan" in filename:
                y_arr = [val**1 for val in y_coup]
            else:
                y_arr = [val**yexp for val in y_coup]
                
            ax.fill(data.T[0], y_arr, color="gainsboro",zorder=zorder)
            ax.plot(data.T[0], y_arr, color="dimgray"  ,zorder=zorder,lw=1)
            zorder+=1
        ...
        for setup in setups:
            ...
            m, c = np.meshgrid(masses, couplings)
            c = [val**1 for val in c]

            # nsig_cut = list()
            # for elem in nsignals:
            #     array = [x for x in elem]
            #     for i in np.arange(len(array)):#go thru, backwards
            #         j=len(array)-(i+1) #j starts on the last element, and ends on 0
            #         if j==0: continue #if we've gotten onto the first element
            #         if array[j]==array[j-1]:
            #             array[j]=0. # if duplicate, set 0 from rear
            #     nsig_cut.append(array)                 
            #     # nsig_cut.append(elem)
            
            
            if False:
                for i,cval in enumerate(c):
                    print(f"m[i]: {m[i]}\tc: {cval}")
                print(f"np.array(nsignals).T+1e-20 : {np.array(nsignals).T+1e-20}")
                
            n = np.log10(np.array(nsignals).T+1e-20)
            # n = np.log10(np.array(nsig_cut).T+1e-20)
            ...
            if False:
                for massindex,nsigs in enumerate(nsignals):
                    if masses[massindex]>2.5 and masses[massindex]<3:
                        print(f"mass:\t{masses[massindex]}")
                        for coupindex,nsig in enumerate(nsigs):
                            print(f"tanB:\t{couplings[coupindex]}\tnsig:\t{nsig}")
        ...
        return plt
#custom to do heat version instead of contour
    def plot_reach_heat(self,
            setups, bounds, projections, bounds2=[],
            title=None, linewidths=None, xlabel=r"Mass [GeV]", ylabel=r"Coupling",
            xlims=[0.01,1],ylims=[10**-6,10**-3], figsize=(7,5), legendloc=None,
            branchings=None, branchingsother=None,
            fs_label=10,ptsize=20,dpi=100,xscale="log",yscale="log",cnorm="linear"
        ):
        ...
        if False:
            ...
            for i,fixedmassslice in enumerate(nsignals):
                for j,num in enumerate(fixedmassslice):
                    if (masses[i]>=2.6 and masses[i]<=3) and (couplings[j]>=1e3 and couplings[j]<=1e5):
                        print(f"mass: {masses[i]}\ttanB: {couplings[j]:e}\tn: {num}")
        ...
        plt.scatter(mcopy,ccopy,c="black",marker=',',s=2*ptsize)
        plt.scatter(m,c,c=[10**v for v in n],cmap="rainbow",marker=',',s=ptsize,norm=cnorm)
        ...
        plt.colorbar(label="nsignals")
        plt.grid()
        return plt

#custom ver to plot where data is a 3-col (mass tanb nsignals) and not a x,y lattice with z values
    def plot_reach_pbp(self,
            setups, bounds, projections, bounds2=[],
            title=None, linewidths=None, xlabel=r"Mass [GeV]", ylabel=r"Coupling",
            xlims=[0.01,1],ylims=[10**-6,10**-3], figsize=(7,5), legendloc=None,
            branchings=None, branchingsother=None,
            fs_label=14,xscale="log",yscale="log",cnorm="log",ptsize=10,dpi=100,
            use_these_indices=[]
        ):
       ...
       # forward experiment sensitivity
        for setup in setups:
            ...
            # print(f"len masses {len(masses)}\nlen coup {len(couplings)}\nlen nsig {len(nsignals)}")
            
            m=[masses[i] for i in use_these_indices]# for i,var in enumerate(masses) if i%use_only_every==0]
            c=[couplings[i] for i in use_these_indices]# for i,var in enumerate(couplings) if i%use_only_every==0]
            n=nsignals
        ...
        if False:
            for i,num in enumerate(n):
                if num>=1e4*level:
                    print(f"i {i}\tm {m[i]}\ttb {c[i]}\tctau {self.model.get_ctau(m[i],c[i])}\tn {num} <<<=======")
                elif num>=1e2*level:
                    print(f"i {i}\tm {m[i]}\ttb {c[i]}\tctau {self.model.get_ctau(m[i],c[i])}\tn {num} <--")
                elif num>=level:
                    print(f"i {i}\tm {m[i]}\ttb {c[i]}\tctau {self.model.get_ctau(m[i],c[i])}\tn {num} ")
        ...
        plt.scatter(mcopy,ccopy,c="black",marker=',',s=10*ptsize)
        plt.scatter(m,c,c=n,cmap="rainbow",marker=',',s=ptsize,norm=cnorm)      
        plt.title(title,fontsize=10)
        plt.xlabel(xlabel,fontsize=10)
        plt.ylabel(ylabel,fontsize=10)
        plt.xscale(xscale)
        ...
        for bound in bounds:
            ...
            plt.fill(data.T[0], y_arr, color="gainsboro",zorder=zorder)
            plt.plot(data.T[0], y_arr, color="dimgray"  ,zorder=zorder,lw=1)
            zorder+=1
            
            if label is None: continue
            plt.text(posx, posy, label, fontsize=fs_label, color="dimgray", rotation=rotation)
        
        return plt, n
        
    def plot_reach_pbp_candidate(self,
            setups, bounds, projections, bounds2=[],
            title=None, linewidths=None, xlabel=r"Mass [GeV]", ylabel=r"Coupling",
            xlims=[0.01,1],ylims=[10**-6,10**-3], figsize=(7,5), legendloc=None,
            branchings=None, branchingsother=None,
       ...
       # forward experiment sensitivity
        ...
        print(f"Number of candiate events: {len(mcopy)}")
        plt.scatter(mcopy,ccopy,c="black",marker='.',s=10*ptsize)
            ...
            for igen, generator in enumerate(generators):
                ...
                for mass in masses:
                    ...
                    for channel in channels:
                        filename = dirname+energy+"TeV_"+channel+"_"+generator.replace("<","LT")+"_m_"+str(mass)+".npy"
                        try:
    ...
    def plot_production_pbp(self,
        ...
        for production in productions:
            ...
            for igen, generator in enumerate(generators):
                ...
                for mass in masses:
                    DEBUGHERE = False
                    if DEBUGHERE: print(production,"\t",mass)
                    if mass<massrange[0]: continue
                    if mass>massrange[1]: continue
                    # loop over channels
                    total = 0
