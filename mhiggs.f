...
*              2,3,6,7   m_a1^2 < 0
*              4,5,6,7   m_h+^2 < 0
*              15        ! wolf - h1/a1 masses >thresholds, event not light
*
...
************************************************************************
      ...
      DOUBLE PRECISION P1MMIN,P1MMAX,S1MMIN,S1MMAX ! wolf
...
*   Comparison:
      ...
      IF(MOD(PFLAG,3).EQ.0)THEN
       ...
       IF(P1MMAX.GT.0D0.AND.PMASS(1).GT.P1MMAX)IFAIL=15
       IF(P1MMIN.GT.0D0.AND.PMASS(1).LT.P1MMIN)IFAIL=15
       IF(S1MMAX.GT.0D0.AND.SMASS(1).GT.S1MMAX)IFAIL=15
       IF(S1MMIN.GT.0D0.AND.SMASS(1).LT.S1MMIN)IFAIL=15

!       IF(SMASS(1).GT.5d0.AND.PMASS(1).GT.5d0)IFAIL=15 ! wolf

!       IF(PMASS(1).GT.4.5d0)IFAIL=15
!       IF(PMASS(1).LT.0.1d0)IFAIL=15
!       IF(PMASS(1).GT.1.0d0)IFAIL=15

!       IF(PMASS(1).GT.0.36d0)IFAIL=15 ! 10^[-1,-.45]
!       IF(PMASS(1).LT.0.1d0)IFAIL=15

!       IF(PMASS(1).GT.1.26d0)IFAIL=15 ! 10^[-.45,.1]
!       IF(PMASS(1).LT.0.36d0)IFAIL=15 

!       IF(PMASS(1).GT.4.5d0)IFAIL=15  ! 10^[.1,.65]
!       IF(PMASS(1).LT.1.26d0)IFAIL=15 


!       IF(IFAIL.EQ.15)WRITE(0,*)"MA,TANB=",PMASS(1),",",PAR(3)
!       IF(IFAIL.EQ.15)WRITE(0,*)"SMASS(1) = ",SMASS(1)
!        IF(IFAIL.NE.15)WRITE(0,*)"SMASS(1) = ",SMASS(1)
      ENDIF
